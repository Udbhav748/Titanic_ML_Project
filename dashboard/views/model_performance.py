...
from theme import ERROR, MODEL_COLORS, MODEL_SHORT_NAMES, PRIMARY, SECONDARY, page_header, style_fig, takeaway

results = load_stage4_results()
X_test, y_test, y_pred, y_proba, metrics = evaluate_v2_on_test()
model = load_model_v2()

page_header("Model Performance", "Why Logistic Regression was selected as the production model.")
...
with tab1:
    ...
    st.dataframe(
        ...
        .highlight_max(subset=["Accuracy", "F1", "ROC-AUC"], color="#E8F0E9"),
...
with tab4:
    ...
    with col1:
        ...
        fig.add_trace(go.Scatter(x=[0, 1], y=[0, 1], mode="lines", line=dict(color="#B0B0B0", dash="dash"), showlegend=False))
...
with tab5:
    ...
    fig.add_hline(y=y_test.mean(), line_dash="dash", line_color="#B0B0B0")
...
with tab6:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=[0, 1], y=[0, 1], mode="lines", line=dict(color="#B0B0B0", dash="dash"), name="Perfect calibration"))
...
with tab7:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=thresholds, y=precisions, mode="lines", name="Precision", line=dict(color="#eb6834", width=2)))
    fig.add_trace(go.Scatter(x=thresholds, y=recalls, mode="lines", name="Recall", line=dict(color="#1baf7a", width=2)))
    ...
    fig.add_vline(x=0.5, line_dash="dash", line_color="#8a8a8a")
...
with st.container(border=True):
    st.markdown(
        "**Logistic Regression — the selected model**\n\n"
        "- Best on 4 of 5 test metrics — accuracy, recall, F1, and ROC-AUC — against a tuned baseline\n"
        "- The only model with no overfitting signature; test performance did not drop below training performance\n"
        "- Its weakest cross-validation fold still outperformed the typical fold of both tree-based alternatives\n"
        "- Roughly 4x faster inference than the tuned baseline, with no hyperparameter search required\n"
        "- Fully interpretable — every prediction decomposes into coefficient-based contributions, not a black box"
    )
